    ...
    def translate_action_to_coords(self, action_json):
        ...
        for action_key, direction in action_json.items():
            ...
            if direction in direction_map and direction_map[direction] is not None:
                ...
                # Clamp to physical limits to prevent dislocation
                # Arms: x,z [-0.6, 0.6], y [-0.3, 0.3] | Legs: x,y [-0.3, 0.3], z [-1, 0.1]
                ...
                # If "stay" or unknown, we just keep the current position
                new_targets[limb_mapping[action_key]] = current_pos.tolist()
        
        return new_targets

    def set_targets(self, action_json):
        if not action_json: return
        # NEW: Convert discrete directions to coordinates
