...
class HumanSwimmingEnv(MujocoEnv):
    def __init__(self, render_mode=None):
        ...
        self._model_path = 'reinforcement_learning/human/simplified_human/human_arms_legs.xml'
        super().__init__(
            self._model_path, 
            frame_skip=5, 
        ...
        obs_size -= 10 # retoactivly adapted to visualize saved training files 
    ...
    def _get_obs(self):
        ...
        return np.concatenate([self.data.qpos, self.data.qvel[-10:]]).astype(np.float32)
...
# # simulate random movement to test the xml setup
# if __name__ == "__main__":
